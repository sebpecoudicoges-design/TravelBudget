...
begin
  ...
  v_target_uid := v_member.auth_user_id;
  if v_target_uid is null and coalesce(v_member.email, '') <> '' then
    select u.id
      into v_target_uid
    from auth.users u
    where lower(u.email) = lower(trim(v_member.email))
  ...
  v_source_message_id := 'trip-payer-approval:' || v_exp.id::text || ':' || v_target_uid::text;
  v_raw := 'Validation paiement Trip - ' || coalesce(v_trip.name, 'Trip') || ' - ' || coalesce(v_exp.label, 'Dépense') || ' - ' || v_exp.amount::text || ' ' || v_exp.currency;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'payer_member_id', v_member.id,
      'payer_member_name', v_member.name,
      'payer_share_amount', v_share_amount,
      'created_by', v_uid,
...
begin
  ...
  if v_exp.transaction_id is null then
    v_payment_tx := public.apply_transaction_v2(
      ...
      p_type => 'expense',
      p_label => '[Trip] Avance - ' || v_exp.label,

-- Recurrence cadence follows occurrence_date, while budget allocation remains
-- independently editable and may span several days.
...
begin
  ...
  v_occurrence_date := coalesce(new.occurrence_date, new.date_start, new.budget_date_start);
  v_budget_start := coalesce(new.budget_date_start, v_occurrence_date);
  v_budget_end := coalesce(new.budget_date_end, v_budget_start);
  if v_occurrence_date is null or v_budget_start is null then
    raise exception 'generated transaction requires occurrence and budget dates';
  end if;
  if v_budget_end < v_budget_start then
    raise exception 'budget end date cannot precede budget start date';
  end if;

  v_period_id := public.get_period_for_travel_date(v_rule.travel_id, v_budget_start);
  if v_period_id is null then
    raise exception 'no budget period for recurring budget date %', v_budget_start;
  end if;

  new.travel_id := v_rule.travel_id;
  new.period_id := v_period_id;
  new.occurrence_date := v_occurrence_date;
  new.budget_date_start := v_budget_start;
  new.budget_date_end := v_budget_end;
  return new;
end;
$function$;
...
before insert or update of recurring_rule_id, occurrence_date, budget_date_start,
  budget_date_end, date_start, travel_id, period_id
...
begin
  ...
  where t.id = r.travel_id and t.user_id = r.user_id;

  if not found then raise exception 'travel not found for recurring rule'; end if;
  if v_travel_end is null then raise exception 'travel.end_date is required'; end if;

  -- A period change may require relinking, but never changes a user's budget
  -- allocation dates. The trigger resolves period_id from budget_date_start.
  update public.transactions t
  set period_id = public.get_period_for_travel_date(
        r.travel_id,
        coalesce(t.budget_date_start, t.occurrence_date, t.date_start)
      ),
  ...
  where t.recurring_rule_id = r.id
    ...
    and public.get_period_for_travel_date(
      r.travel_id,
      coalesce(t.budget_date_start, t.occurrence_date, t.date_start)
    ) is not null;
  ...
  if r.end_date is not null and r.end_date < v_horizon then v_horizon := r.end_date; end if;
  ...
  while v_due is not null and v_due <= v_horizon loop
    if r.max_occurrences is not null and v_occurrence_count >= r.max_occurrences then exit; end if;

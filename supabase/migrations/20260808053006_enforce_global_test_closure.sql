-- A globally closed scenario is immutable for browser test accounts.
...
with check (
  ...
  and exists (
    ...
    where s.id = scenario_id and s.campaign_id = campaign_id and s.closed_at is null
  )
);
...
using (
  ...
  and exists (
    select 1 from public.app_test_scenarios s
    where s.id = scenario_id and s.closed_at is null
  )
)
with check (
  user_id = (select auth.uid())

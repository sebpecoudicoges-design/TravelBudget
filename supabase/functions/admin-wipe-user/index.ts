...
serve(async (req) => {
  ...
  try {
    ...
    for (const step of [
      ...
      admin.from("sport_sets").delete().eq("user_id", targetUserId),
      admin.from("sport_session_items").delete().eq("user_id", targetUserId),
      admin.from("sport_sessions").delete().eq("user_id", targetUserId),

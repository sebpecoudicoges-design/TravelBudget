...
(function () {
  ...
  function renderNutrition(reason) {
    ...
    if (!CACHE.loaded && !CACHE.loading) {
      loadNutrition().then((changed) => {
        if (changed && (window.activeView || "") === "nutrition") renderNutrition("loaded");
      }).catch(() => {});
    }
        ...
        <div class="tb-work-grid" style="display:grid;grid-template-columns:minmax(280px,390px) 1fr;gap:14px;margin-top:14px;">
          <div style="border:1px solid var(--border);border-radius:8px;padding:12px;background:var(--panel2);">
            ...
            <div class="row" style="gap:10px;">
              <div class="field" style="flex:1;"><label>${esc(txt("Grammes", "Grams"))}</label><input id="nutrition-grams" type="number" min="0" step="5" value="100"></div>
  ...
  function updateNutritionPreview(root) {
    ...
    const values = nutritionForGrams(food, n(root.querySelector("#nutrition-grams")?.value, food?.servingGrams || 100));
    const out = root.querySelector("#nutrition-preview");
    if (out) out.textContent = `${Math.round(values.kcal)} kcal · P ${fmtMacro(values.protein)} · G ${fmtMacro(values.carbs)} · L ${fmtMacro(values.fat)}`;
  }
  function bindNutrition(root) {
    const search = root.querySelector("#nutrition-search");
    if (search) search.oninput = () => { CACHE.foodQuery = search.value || ""; renderNutrition("search"); };
    ["#nutrition-food", "#nutrition-grams"].forEach(sel => {
      const el = root.querySelector(sel);
      if (el) el.oninput = () => updateNutritionPreview(root);
      if (el) el.onchange = () => updateNutritionPreview(root);
    });
  ...
  async function saveNutritionMeal(root) {
    ...
    const waterMl = n(root.querySelector("#nutrition-water")?.value, 0);
    ...
    try {
      if (c && uid()) {
        ...
        rows.unshift({
          ...
          item: { id: itemId, user_id: uid(), meal_id: mealId, food_key: food.key, label: food.name, grams, kcal: nut.kcal, protein_g: nut.protein, carbs_g: nut.carbs, fat_g: nut.fat, fiber_g: nut.fiber, created_at: new Date().toISOString() },
        });
        saveLocalMeals(rows);
      }

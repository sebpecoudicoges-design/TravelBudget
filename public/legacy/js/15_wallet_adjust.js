...
async function adjustWalletBalance(walletId) {
  await safeCall("Ajuster solde", async () => {
    ...
    const raw = prompt(
      `Solde actuel pour "${w.name}" (${w.currency}) :\n\n(Info: on recalcule le solde de base pour que le solde affiché corresponde, en tenant compte des transactions déjà payées.)`,
      String(effectiveNow)
    );
    if (raw === null) return;
    ...
    // Recalibrate baseline so that displayed balance (baseline + paid tx) == val
    let paidDelta = 0;
    try {

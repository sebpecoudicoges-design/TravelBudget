...
- **Vue d'ensemble** : prévu, réel payé, écart par devise, échéances à surveiller et un hero analytique par abonnement.
...
- La sélection Transactions rattache ou détache jusqu'à 500 lignes atomiquement via `link_transactions_to_recurring_rule` ; les échéances générées restent protégées.

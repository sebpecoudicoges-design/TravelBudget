...
- [x] Ajouter un workflow AAB Play Store : `android:bundle-check`, `android:bundle-release`, signature par variables locales et verification `jarsigner`.
...
- [x] Reprendre Alimentation en profondeur en `10.5.344` : quatre espaces internes accessibles, journal d hydratation horodate, recuperation Sommeil/Alcool, historique detaille et suppression du tableau Semaine active devenu mort.
  ...
  - [x] Livrer en `10.5.354` la file A associer explicable, la detection de doublon, la fiche abonnement et les onglets mobiles 2 x 2.
...
- [x] Nutrition : separer catalogue, journal, objectifs et historique.
  ...
  - [x] Supprimer les exports globaux Nutrition inutilises `tbSaveHealthGoal` et `tbHealthGoalTargets`, tout en conservant `tbLoadHealthGoal` pour les KPI.
    ...
    - [x] Corriger en 10.5.363 la conservation du brouillon lors de l ajout ou de la suppression d une ligne ingrédient dans la fenêtre recette.
...
- [x] Decouper les gros bundles par domaine.
  ...
  - [x] Extraire le rendu du plan du builder Sport vers `sportFormView.js`, avec contrôles modifier/deplacer/supprimer et chips progression couverts par tests.
  ...
  - [x] Supprimer les anciens exports Nutrition de sauvegarde/calcul objectif sans appel ; Nutrition 187.0 KiB.
...
- [ ] Executer la stabilisation module par module 10.5.316 selon `docs/STABILIZATION_CHECKLIST_10.5.316.md`, puis reprendre Play Store seulement apres les retests.
  ...
  - [x] Traiter en 10.5.325 le premier affichage Dashboard, les aides persistantes, la grille Compte compacte et les retours Transactions rouge/vert ; clore globalement les tests deja valides et creer quatre retests derives.
  - [x] Corriger en 10.5.326 l ordre du boot protege : attendre le role serveur, monter Dashboard, puis synchroniser ; clore les quatre retests valides et enchainer le retest Dashboard en echec.
  ...
  - [x] Supprimer les fallbacks HTML Documents restants au profit du helper unique `docView(...)` : shell, dossiers, cartes, apercu, infos, partage, batch, transactions et assets ne gardent plus de templates concurrents dans `43_documents_ui.js`, passe de 75.28 a 74.09 KiB.

...
for (const width of [1440, 390]) for (const theme of ['light', 'dark']) {
  test(`audit cards ${width}px ${theme}`, async ({ page }) => {
    ...
    await page.evaluate(async theme => {
      ...
      const periods = await import('/src/features/analysis/analysisPeriodRules.js');
      const comparison = await import('/src/features/analysis/analysisPeriodComparison.js');
      ...
      document.querySelector('main').insertAdjacentHTML('afterbegin', `<div class="analysis-toolbar" style="margin-bottom:14px"><div class="analysis-filter-group"><label for="audit-period">Période</label><select id="audit-period">${window.TBAnalysisFilterView.renderPeriodFilterOptions({ activeLabel: 'Période active' })}</select></div><div class="analysis-filter-group"><label for="audit-comparison">Comparaison</label><select id="audit-comparison"><option>Sans comparaison</option><option selected>Période précédente</option></select></div></div>`);
      document.querySelector('section').insertAdjacentHTML('beforeend', unpaidBlock);
      document.querySelector('main').insertAdjacentHTML('beforeend', comparison.renderPeriodComparison({ current: { base: 'AUD', start: '2026-09-14', end: '2026-09-20' }, previous: { spentToToday: 250 }, range: periods.previousComparableRange({ start: '2026-09-14', elapsedDays: 6 }), metrics: periods.comparisonMetrics({ avgPerDay: 40 }, { avgPerDay: 50 }), formatCurrency: n => `${n.toFixed(2)} AUD` }));
    }, theme);
    await expect(page.locator('.tb-analysis-comparison')).toContainText('Face à');
    await expect(page.locator('.tb-analysis-comparison')).toContainText('-20.0 %');

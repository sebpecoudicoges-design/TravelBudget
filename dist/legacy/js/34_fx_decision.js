...
(function () {
  ...
  function readDecisionInputs(rows) {
    ...
    return { weeklyIncomeAud, mode, rate, weeklyIncomeEur, eurNeedAmount, audSafetyAmount, eurNeedRatio, localAudSafetyRatio };
  }

  function buildRecommendationExplanation(decision, inputs) {
    const metrics = decision.metrics || {};
    const pos90 = Number(metrics.position90 || 0);
    const trend = Number(metrics.trendRatio || 0);
    const volatility = Number(metrics.volatility90 || 0);
    const parts = [];
    if (pos90 >= 0.01) parts.push(T("fxdecision.reason.rate_strong", { percent: (pos90 * 100).toFixed(2) }));
    else if (pos90 <= -0.01) parts.push(T("fxdecision.reason.rate_weak", { percent: Math.abs(pos90 * 100).toFixed(2) }));
  ...
  function renderCard(host, decision, rows, source, inputs) {
    ...
    const pos90Pct = Number(metrics.position90 || 0) * 100;
    const volPct = Number(metrics.volatility90 || 0) * 100;
    ...
    const coverageStart = rows[0]?.date || "";
    const coverageEnd = rows[rows.length - 1]?.date || "";
    const count90 = normalizeAudEurRows(rows).filter((row) => String(row.date) >= addDaysISO(todayISO(), -90)).length;
    ...
    const controlsHtml = scaleMode === "amount" ? `
          ...
          <div class="field">
            <label>${esc(T("fxdecision.aud_safety_amount"))}</label>
      ...
      <section class="card" style="margin-top:12px;overflow:hidden;">
        ...
        <div style="display:grid;grid-template-columns:minmax(0,1.4fr) minmax(280px,.8fr);gap:16px;margin-top:14px;">
          <div style="min-width:0;">
            ...
            <div class="muted" style="font-size:12px;margin-top:8px;">${esc(T("fxdecision.coverage", { count: rows.length, count90, start: coverageStart, end: coverageEnd }))}</div>
          </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:8px;margin-top:12px;" class="muted">
          ...
          <span>${esc(T("fxdecision.avg90"))}: <strong>${Number(metrics.avg90 || 0).toFixed(4)}</strong></span>
          <span>${esc(T("fxdecision.pos90"))}: <strong>${pos90Pct.toFixed(2)}%</strong></span>
    ...
    ["tb-fx-decision-income", "tb-fx-decision-scale-mode", "tb-fx-decision-eur-need", "tb-fx-decision-aud-safety", "tb-fx-decision-eur-need-amount", "tb-fx-decision-aud-safety-amount"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) {
        el.onchange = rerender;
        el.oninput = id === "tb-fx-decision-income" || id === "tb-fx-decision-scale-mode" ? null : rerender;
      }
    });

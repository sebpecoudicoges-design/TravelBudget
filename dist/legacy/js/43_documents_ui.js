...
(function(){
  ...
  let CACHE = { folders: [], documents: [], selectedFolderId: '', loading: false, error: '', search: '' };
  ...
  function visibleDocs(){
    ...
    if(q) rows = rows.filter(d => `${d.name||''} ${d.original_filename||''} ${d.mime_type||''}`.toLowerCase().includes(q));
    return rows;
  }
  ...
  function renderFolders(){
    ...
    return `<div class="tb-doc-sidebar">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><strong>Dossiers</strong><button class="btn" type="button" onclick="window.tbDocumentsCreateFolder()">+ Dossier</button></div>
      <button class="tb-doc-folder${allActive}" type="button" onclick="window.tbDocumentsSelectFolder('')"><span>📁 Tous les documents</span><small>${esc((CACHE.documents||[]).length)}</small></button>
      ${folders.map(f=>`<button class="tb-doc-folder${String(f.id)===String(CACHE.selectedFolderId)?' active':''}" type="button" onclick="window.tbDocumentsSelectFolder('${esc(f.id)}')"><span>📂 ${esc(f.name)}</span><small>${esc(folderCount(f.id))}</small></button>`).join('')}
    </div>`;
  }
  ...
  function renderDocCard(d){
    ...
    const icon = isPdf(mime,name) ? '📄' : (isImg(mime) ? '🖼️' : '📎');
    return `<article class="tb-doc-card" data-doc-id="${esc(d.id)}">
      <div class="tb-doc-thumb">${icon}</div>
      <div class="tb-doc-name">${esc(name)}</div>
      <div class="tb-doc-meta"><span>${esc(fmtDate(d.created_at))}</span><span>·</span><span>${esc(fmtSize(d.size_bytes))}</span></div>
  ...
  function renderMain(){
    ...
    return `<div class="tb-doc-main">
      <div class="tb-doc-toolbar">
        <div><strong>${esc(folder ? folder.name : 'Tous les documents')}</strong><div class="muted" style="font-size:12px;">${esc(docs.length)} document(s)</div></div>
        <input id="tb-doc-search" class="tb-doc-search" type="search" value="${esc(CACHE.search||'')}" placeholder="Rechercher par nom…" oninput="window.tbDocumentsSetSearch(this.value)" />
      </div>
      ${CACHE.loading ? '<div class="tb-doc-empty">Chargement des documents…</div>' : ''}
      ${CACHE.error ? `<div class="tb-doc-empty"><strong>Module Documents non initialisé.</strong><br><br>${esc(CACHE.error)}<br><br><span>Applique d’abord le patch SQL V9.6.5, puis recharge l’application.</span></div>` : ''}
      ${(!CACHE.loading && !CACHE.error && !docs.length) ? '<div class="tb-doc-empty">Aucun document ici. Crée un dossier ou ajoute ton premier PDF / image.</div>' : ''}
      ${(!CACHE.loading && !CACHE.error && docs.length) ? `<div class="tb-doc-grid">${docs.map(renderDocCard).join('')}</div>` : ''}
    </div>`;
  }
  ...
  async function upload(files){
    ...
    for(const file of list){
      try{
        ...
        const ins = await c.from(table('documents','documents')).insert({ id: docId, user_id: uid, folder_id: folderId, name: baseName || 'Document', original_filename: file.name || safe, storage_bucket: BUCKET, storage_path: path, mime_type: file.type || '', size_bytes: file.size || 0 });
        if(ins.error) throw ins.error;
      }catch(e){ console.warn('[TB][documents] upload failed', e); alert(`Upload impossible pour ${file.name || 'document'} : ${e.message || e}`); }
    }
    await ensureLoaded();
  }
